...
contextBridge.exposeInMainWorld("Electron", {
  ...
  contracts: {
    ...
    upload: (file: any) => ipcRenderer.invoke(IPC_CHANNELS.DATA.CONTRACTS.UPLOAD, file),
    archive: (id: string) => ipcRenderer.invoke(IPC_CHANNELS.DATA.CONTRACTS.ARCHIVE, id),
    updateAccessControl: (id: string, data: any) => ipcRenderer.invoke(IPC_CHANNELS.DATA.CONTRACTS.UPDATE_ACCESS_CONTROL, id, data),
    assignUsers: (id: string, users: any) => ipcRenderer.invoke(IPC_CHANNELS.DATA.CONTRACTS.ASSIGN_USERS, id, users),
    getById: (id: string) => ipcRenderer.invoke('contracts:getById', id),
  },
  // Suplementos
  supplements: {
    list: (contractId: string) => ipcRenderer.invoke(IPC_CHANNELS.DATA.SUPPLEMENTS.LIST, contractId),
    create: (contractId: string, data: any) => ipcRenderer.invoke(IPC_CHANNELS.DATA.SUPPLEMENTS.CREATE, contractId, data),
    update: (id: string, data: any) => ipcRenderer.invoke(IPC_CHANNELS.DATA.SUPPLEMENTS.UPDATE, id, data),
    delete: (id: string) => ipcRenderer.invoke(IPC_CHANNELS.DATA.SUPPLEMENTS.DELETE, id),
    export: (id: string) => ipcRenderer.invoke(IPC_CHANNELS.DATA.SUPPLEMENTS.EXPORT, id),
    upload: (file: any) => ipcRenderer.invoke(IPC_CHANNELS.DATA.SUPPLEMENTS.UPLOAD, file),
  },
  // Documentos
  documents: {
    list: (filters?: any) => ipcRenderer.invoke(IPC_CHANNELS.DATA.DOCUMENTS.LIST, filters),
    ...
    getByContract: (contractId: string) => ipcRenderer.invoke(IPC_CHANNELS.DATA.DOCUMENTS.GET_BY_CONTRACT, contractId),
    getBySupplement: (supplementId: string) => ipcRenderer.invoke(IPC_CHANNELS.DATA.DOCUMENTS.GET_BY_SUPPLEMENT, supplementId),
    open: (id: string) => ipcRenderer.invoke(IPC_CHANNELS.DATA.DOCUMENTS.OPEN, id),
  },
  // Usuarios
  users: {
    ...
    toggleActive: (id: string) => ipcRenderer.invoke(IPC_CHANNELS.DATA.USERS.TOGGLE_ACTIVE, id),
    changePassword: (data: any) => ipcRenderer.invoke(IPC_CHANNELS.DATA.USERS.CHANGE_PASSWORD, data),
    getById: (id: string) => ipcRenderer.invoke('users:getById', id),
  },
  // Roles
  roles: {
    list: () => ipcRenderer.invoke(IPC_CHANNELS.DATA.ROLES.LIST),
    create: (data: any) => ipcRenderer.invoke(IPC_CHANNELS.DATA.ROLES.CREATE, data),
    update: (data: any) => ipcRenderer.invoke(IPC_CHANNELS.DATA.ROLES.UPDATE, data),
    delete: (id: string) => ipcRenderer.invoke(IPC_CHANNELS.DATA.ROLES.DELETE, id),
  },
  // Estadísticas
  statistics: {
  ...
  // Notificaciones
  notifications: {
    show: (options: { title: string; body: string }) => ipcRenderer.invoke(IPC_CHANNELS.NOTIFICATIONS.SHOW, options),
    clear: (id: string) => ipcRenderer.invoke(IPC_CHANNELS.NOTIFICATIONS.CLEAR, id),
    markRead: (id: string) => ipcRenderer.invoke(IPC_CHANNELS.NOTIFICATIONS.MARK_READ, id),
    getUnread: () => ipcRenderer.invoke(IPC_CHANNELS.NOTIFICATIONS.GET_UNREAD),
  },
  // Backups
  backups: {
    create: (description?: string) => ipcRenderer.invoke('backups:create', description),
  ...
  theme: {
    ...
    setAppTheme: (theme: 'light' | 'dark' | 'system') => ipcRenderer.invoke('theme:set-app', theme),
  },
  // API genérica
  api: {
    request: (req: { method: string; endpoint: string; data?: any; params?: any }) => ipcRenderer.invoke('api:request', req),
  },
  // IPC seguro
  ipcRenderer: {
