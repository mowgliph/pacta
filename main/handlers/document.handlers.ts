...
export function registerDocumentHandlers(eventManager: EventManager): void {
  const handlers: IpcHandlerMap = {
    ...
    [IPC_CHANNELS.DATA.DOCUMENTS.GET_BY_CONTRACT]: async (event, contractId) => {
      logger.info('Documentos por contrato solicitado', { contractId });
      try {
        const documents = await prisma.document.findMany({ where: { contractId } });
        return documents;
      } catch (error) {
    ...
    [IPC_CHANNELS.DATA.DOCUMENTS.GET_BY_SUPPLEMENT]: async (event, supplementId) => {
      logger.info('Documentos por suplemento solicitado', { supplementId });
      try {
        const documents = await prisma.document.findMany({ where: { supplementId } });
        return documents;
      } catch (error) {
    ...
    [IPC_CHANNELS.DATA.DOCUMENTS.UPLOAD]: async (event, meta, attachment) => {
      logger.info('Subida de documento solicitada', { meta, fileName: attachment?.name });
      try {
        ...
        const parsed = DocumentSchema.parse({ ...meta, name: attachment.name, type: attachment.type });
    ...
    [IPC_CHANNELS.DATA.DOCUMENTS.DOWNLOAD]: async (event, id) => {
      ...
      try {
        ...
        if (!doc || !doc.filePath) throw new Error('Documento no encontrado');
        const absPath = path.resolve(DOCUMENTS_DIR, doc.filePath);
        if (!fs.existsSync(absPath)) throw new Error('Archivo no encontrado en disco');
        await prisma.document.update({ where: { id }, data: { downloads: { increment: 1 } } });
        return { path: absPath };
      } catch (error) {
