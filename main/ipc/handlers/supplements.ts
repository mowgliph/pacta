...
export function setupSupplementHandlers(): void {
  ...
  withErrorHandling(
    IPC_CHANNELS.SUPPLEMENTS_GET_BY_CONTRACT,
    async (_, contractId: string) => {
      try {
        if (!contractId) {
          throw ErrorHandler.createError(
            "ValidationError",
            "El ID del contrato es requerido"
          );
        }
  ...
  withErrorHandling(
    IPC_CHANNELS.SUPPLEMENTS_GET_BY_ID,
    async (_, supplementId: string) => {
      try {
        if (!supplementId) {
          throw ErrorHandler.createError(
            "ValidationError",
            "El ID del suplemento es requerido"
          );
        }

        const supplement = await SupplementService.getSupplementById(
          supplementId
        );
  ...
  withErrorHandling(
    IPC_CHANNELS.SUPPLEMENTS_CREATE,
    async (_, supplementData) => {
      try {
        ...
        const newSupplement = await SupplementService.createSupplement(
          validatedData,
          supplementData.userId
        );

        logger.info(
          `Suplemento creado: ${newSupplement.id} para contrato ${newSupplement.contractId}`
        );
        ...
        if (error instanceof z.ZodError) {
          ...
          logger.warn(
            "Error de validación al crear suplemento:",
            validationErrors
          );
          throw ErrorHandler.createError(
            "ValidationError",
  ...
  withErrorHandling(IPC_CHANNELS.SUPPLEMENTS_UPDATE, async (_, updateData) => {
    try {
      ...
      if (error instanceof z.ZodError) {
        ...
        logger.warn(
          "Error de validación al actualizar suplemento:",
          validationErrors
        );
        throw ErrorHandler.createError(
          "ValidationError",
  ...
  withErrorHandling(
    IPC_CHANNELS.SUPPLEMENTS_APPROVE,
    async (_, supplementId: string, userId: string) => {
      try {
        if (!supplementId) {
          throw ErrorHandler.createError(
            "ValidationError",
        ...
        const approvedSupplement = await SupplementService.approveSupplement(
          supplementId,
          userId
        );

        logger.info(
          `Suplemento aprobado: ${supplementId} por usuario ${userId}`
        );
  ...
  withErrorHandling(
    IPC_CHANNELS.SUPPLEMENTS_DELETE,
    async (_, supplementId: string, userId: string) => {
      try {
        if (!supplementId) {
          throw ErrorHandler.createError(
        ...
        const result = await SupplementService.deleteSupplement(
          supplementId,
          userId
        );

        logger.info(
          `Suplemento eliminado: ${supplementId} por usuario ${userId}`
        );
        return result;
      } catch (error) {
