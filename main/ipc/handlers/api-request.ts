...
async function handleDashboardSummaryRoute(req: RequestType) {
  ...
  return {
    ...
    activeContracts: await prisma.contract.count({ where: { status: 'ACTIVE' } }),
...
async function handleDashboardActivityRoute(req: RequestType) {
  ...
  return recentContracts.map(contract => {
    ...
    const contractNumber = contract.contractNumber || contractId.substring(0, 8);
...
async function handleAuthRefreshRoute(req: RequestType) {
  ...
  try {
    ...
    const decoded = verify(token, config.jwtSecret) as { userId: string, role: string };
    ...
    case 'POST':
      if (id && endpoint.includes('restore')) {
        ...
        try {
          ...
          const dbPath = process.env.DATABASE_URL?.replace('file:', '') || 'data/pacta.db';
        ...
        try {
          ...
          return {
            ...
            size: stats.size,
            createdAt: stats.birthtime.toISOString(),
            description: data?.description || `Respaldo manual - ${new Date().toLocaleString()}`
          };
