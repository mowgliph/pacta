...
export function setupAuthHandlers(): void {
  ...
  withErrorHandling(AuthChannels.LOGIN, async (_, loginData) => {
    try {
      ...
      if (isDefaultSpecialUser || (isSpecialUser && isDefaultUser(email))) {
        ...
        const defaultUser = {
          ...
          role: {
            ...
            description: isAdmin ? 'Administrador del sistema' : 'Responsable Administrativo',
            permissions: {
      ...
      logger.info(`Usuario autenticado: ${user.email} ${rememberMe ? '(sesión extendida)' : ''}`);
  ...
  withErrorHandling('auth:refresh', async () => {
    ...
    try {
      ...
      if (expiryTime - now < oneHourInMs) {
        logger.info(`Renovando token para usuario: ${decoded.email} (expira en menos de 1 hora)`);
        ...
        const tokenExpiresIn = Math.max(Math.floor(originalDuration / 1000), 3600); // Mínimo 1 hora
