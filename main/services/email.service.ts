...
export class EmailService {
    ...
    try {
      ...
      if (correoConfig) {
        ...
        if (process.env.SMTP_HOST) {
          ...
          this.logger.info("Transporte de email configurado desde variables de entorno");
        } else {
          this.logger.warn("No se encontró configuración de email. La funcionalidad de envío estará desactivada.");
        }
      }
    } catch (error) {
  ...
  async sendEmail(options: {
    ...
    try {
      ...
      if (!this.transporter) {
        await this.logEmailAttempt(options.to, options.subject, options.html || "", "failed", "Email service not configured");
        return false;
      }
      ...
      const from = correoConfig?.from || process.env.SMTP_FROM || "PACTA <noreply@pacta.com>";
      ...
      this.logger.info("Email enviado correctamente", { messageId: info.messageId });
      
      // Registrar email en BD
      await this.logEmailAttempt(options.to, options.subject, html || "", "sent");
      ...
      await this.logEmailAttempt(options.to, options.subject, options.html || "", "failed", errorMessage);
   ...
   */
  async sendTemplatedEmailFromDB(
    templateName: string,
    to: string | string[],
    ...
    try {
      ...
      this.logger.error(`Error al enviar email con plantilla "${templateName}":`, error);
      return false;
    }
  }
   ...
   */
  async sendPasswordResetEmail(user: { email: string; name: string }, resetToken: string): Promise<boolean> {
    return await this.sendEmail({
      to: user.email,
   ...
   */
  async sendContractExpirationEmail(user: { email: string; name: string }, contract: { id: string; title: string; endDate: Date }): Promise<boolean> {
    ...
    return await this.sendEmail({
      ...
      context: {
        ...
        daysRemaining,
        contractUrl: `${process.env.APP_URL || "http://localhost:3000"}/contracts/${contract.id}`,
      },
    });
  }
