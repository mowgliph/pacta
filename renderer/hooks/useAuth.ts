...
export function useAuth() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      if (token && !user) {
        const result = await window.Electron.ipcRenderer.invoke("auth:verify", {
          deviceId,
        });
  ...
  useEffect(() => {
    ...
    const interval =
      timeUntilExpiry <= AUTH_CONFIG.REFRESH_THRESHOLD * 2
  ...
  const hasPermission = useCallback(
    (resource: string, action: string): boolean => {
      ...
      if (user.customPermissions) {
        const permissions =
          user.customPermissions[resource as keyof CustomPermissions];
        if (
          permissions &&
          permissions[action as keyof typeof permissions] === true
        ) {
          return true;
        }
      }
