...
import { Contract, ContractFilters, ContractStatus } from "../types/contract";
...
interface ContractState {
  ...
  createContract: (
    contract: Omit<Contract, "id" | "status" | "createdAt" | "updatedAt">
  ) => Promise<string>;
  createSupplement: (
    supplement: Omit<ContractSupplement, "id" | "createdAt">
  ) => Promise<void>;
      ...
      if (newFilters.dateRange) {
        ...
        if (start && end) {
          filtered = filtered.filter((c) => {
            ...
            return (
              contractDate >= new Date(start) && contractDate <= new Date(end)
            );
          });
        }
      }
  ...
  setPagination: (page, pageSize) => {
    set((state) => {
      ...
      const totalPages = Math.ceil(
        state.filteredContracts.length / newPageSize
      );
...
export const selectContractsByStatus =
  (status: Contract["status"]) => (state: ContractState) =>
    state.contracts.filter((c) => c.status === status);

export const selectContractsByType =
  (type: Contract["type"]) => (state: ContractState) =>
    state.contracts.filter((c) => c.type === type);

export const selectUpcomingExpirations =
  ...
  (state: ContractState) => {
    ...
    return state.contracts
      .filter((c) => {
        ...
        return (
          c.status === "Vigente" && endDate > today && endDate <= thresholdDate
        );
      })
      .sort(
