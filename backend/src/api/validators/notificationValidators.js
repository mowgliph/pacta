...
export const createBulkNotificationSchema = () => {
  return z.object({
    userIds: z.array(z.string().uuid('ID de usuario inválido')).min(1, 'Se requiere al menos un usuario'),
    title: z.string().min(1, 'El título es requerido'),
    message: z.string().min(1, 'El mensaje es requerido'),
    type: z.enum(['INFO', 'WARNING', 'ERROR', 'SUCCESS', 'SYSTEM', 'CONTRACT']),
    url: z.string().url('URL inválida').optional(),
...
export const markAsReadSchema = () => {
  return z.object({
    ids: z.union([
      ...
      z.array(z.string().uuid('ID de notificación inválido')).min(1, 'Se requiere al menos una notificación')
    ])
  });
};
...
export const createExpirationNotificationsSchema = () => {
  return z.object({
    daysThreshold: z.number().int().positive().max(365).default(30).or(
