...
export const contractAnalyticsSchema = () => {
  return z.object({
    ...
    endDate: z.string().datetime('Fecha de fin inválida').optional(),
    groupBy: z
      .enum(['status', 'type', 'company', 'department', 'tag', 'month', 'quarter', 'year'])
      .optional(),
...
export const userActivityAnalyticsSchema = () => {
  return z.object({
    ...
    activityType: z
      .enum(['LOGIN', 'CREATE', 'UPDATE', 'DELETE', 'VIEW', 'EXPORT', 'IMPORT', 'OTHER'])
      .array()
      .optional(),
    entityType: z
      .enum(['CONTRACT', 'COMPANY', 'USER', 'DEPARTMENT', 'LICENSE', 'DOCUMENT'])
      .array()
      .optional(),
    groupBy: z
      .enum(['user', 'role', 'activityType', 'entityType', 'day', 'week', 'month'])
      .optional(),
  });
};
