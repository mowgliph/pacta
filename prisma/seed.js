...
async function main() {
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    const contrato =
      (await prisma.contract.findUnique({ where: { contractNumber } })) ||
      (await prisma.contract.create({
        data: {
          ...
          providerObligations: JSON.stringify([`Obligación proveedor ${i}`]),
          clientObligations: JSON.stringify([`Obligación cliente ${i}`]),
